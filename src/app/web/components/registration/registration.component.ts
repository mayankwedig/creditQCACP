...
export class RegistrationComponent implements OnInit {
      ...
      .subscribe(
        result => {
          ...
          if (this.gstStateCode != "08") {
            ...
            this.paymentPost.gst_amount = Math.round(
              (this.settings.registration_fee *
                this.settings.registration_igst) /
                100
            ).toFixed(0);
            this.paymentPost.total_amount = parseFloat(
              this.settings.registration_fee
            );
            this.paymentPost.paid_amount =
              parseFloat(this.paymentPost.gst_amount) +
              parseFloat(this.settings.registration_fee);
          } else {
            //CGST
            // this.paymentPost.gst_per = this.settings.registration_cgst + this.settings.registration_sgst;
            this.paymentPost.cgst_amount = Math.round(
              (this.settings.registration_fee *
                this.settings.registration_cgst) /
                100
            ).toFixed(0);
            this.paymentPost.sgst_amount = Math.round(
              (this.settings.registration_fee *
                this.settings.registration_sgst) /
                100
            ).toFixed(0);
            this.paymentPost.total_amount = parseFloat(
              this.settings.registration_fee
  ...
  // Auto apply RefferalLinkCode
  applyRefferalLinkRefferalCode() {
    if (sessionStorage.getItem("refferalCode")) {
      let item = JSON.parse(sessionStorage.getItem("refferalCode"));
      sessionStorage.removeItem("refferalCode");
      if (item.refferalCode != "") {
        this.haveReferralCode = true;
        this.customerData.referral_code = item.refferalCode;
        this.applyReferralCode();
      } else {
        this.haveReferralCode = false;
        this.customerData.referral_code = "";
      }
    } else {
      this.haveReferralCode = false;
  ...
  gotoBankStep() {
    ...
    this.bankStep = true;
    this.paymentStep = false;
    this.applyRefferalLinkRefferalCode();
  }
  ...
  submitGST() {
    ...
    if (
      ...
      this.customerData.gst_no.length != 15
    ) {
      this.errorMessage = "Please enter valid GST no";
    } else {
      this.userService
        .userByGST(this.customerData)
        .pipe()
        .subscribe(
          result => {
            if (result != null && result.body.length > 0) {
              if (result.body[0].status == UserStatus.active) {
                this.errorMessage =
                  "GST Number already registered with us. Please login using your credential.";
              } else if (
                result.body[0].status == UserStatus.deleted ||
                result.body[0].status == UserStatus.blocked
              ) {
                this.errorMessage =
                  "Your account has been deactivated. Please contact to administrator for more information.";
              } else {
                if (result.body[0]["pan_no"] == "null") {
                  result.body[0]["pan_no"] = "";
                }
                ...
                if (
                  this.customerData.status == UserStatus.inactive ||
              ...
              this.errorMessage =
                'No data available for "' + this.customerData.gst_no + '"';
            }
          },
          error => {
  ...
  submitDetails() {
    this.errorMessage = "";
    if (
      this.customerData.category_id < 0 ||
      this.customerData.category_id == "" ||
      this.customerData.category_id == null
    ) {
      this.errorMessage = "Please select business category";
    } else if (
      this.customerData.category_id == 0 &&
      (this.customerData.category_name == "" ||
        this.customerData.category_name == null)
    ) {
      this.errorMessage = "Please enter business category";
    } else if (
      this.customerData.category_id == 0 &&
      this.customerData.category_name.trim() == ""
    ) {
      this.errorMessage = "Please enter business category";
    } else if (
      this.customerData.confirm_password != this.customerData.password
    ) {
  ...
  submitMobileVerifyStep() {
    ...
    if (this.customerData.mobile_vefify_otp != "") {
      let v_otp_data = {
        ...
        otp: this.customerData.mobile_vefify_otp
      };
      this.userService
      ...
      .subscribe(
        result => {
          ...
          if (result.body.paid_amount > 0) {
            ...
            this.spinnerVisibilityService.hide();
            this.commonService.closePopup();
            this.commonService.openSuccessPopup(
        ...
        .subscribe(
          result => {
            if (result.body) {
              ...
              this.errorMessage_IFSC_code =
                "Invalid IFSC Code or Bank data not found";
            }
          },
          error => {
            this.errorMessage = error.error.message;
  ...
  applyPromo() {
    ...
    this.paymentService
      .applyPromoCode(payData)
      .pipe()
      .subscribe(
        result => {
          if (this.gstStateCode != "08") {
            this.paymentPost.paid_amount =
              this.paymentPost.total_amount - result.body.discount;
            this.paymentPost.gst_amount = Math.round(
              (this.paymentPost.paid_amount * this.settings.registration_igst) /
                100
            ).toFixed(0);
            this.paymentPost.paid_amount =
              parseFloat(this.paymentPost.gst_amount) +
              parseFloat(this.paymentPost.paid_amount);
          } else {
            this.paymentPost.paid_amount =
              this.paymentPost.total_amount - result.body.discount;
            this.paymentPost.cgst_amount = Math.round(
              (this.paymentPost.paid_amount * this.settings.registration_cgst) /
                100
            ).toFixed(0);
            this.paymentPost.sgst_amount = Math.round(
              (this.paymentPost.paid_amount * this.settings.registration_sgst) /
                100
            ).toFixed(0);
            this.paymentPost.gst_amount = Math.round(
              (this.paymentPost.paid_amount *
  ...
  applyReferralCode() {
    ...
    this.errorMessage_Referral = "";
    this.userService
      .applyReferralCode(this.customerData)
      .pipe()
      .subscribe(
        (result: any) => {
  ...
  applyExecutiveCode() {
    ...
    this.errorMessage_Executive = "";
    this.userService
      .applyExecutiveCode(this.customerData.executive_code)
      .pipe()
      .subscribe(
        result => {},
        error => {
