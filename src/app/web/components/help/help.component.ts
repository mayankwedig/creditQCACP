...
export class HelpComponent implements OnInit {
      ...
      .subscribe(
        result => {
          if (result != null) {
            ...
            this.maxPageNo =
              parseInt(
                Math.round(result.recordCount / this.page_size).toFixed(0)
              ) + 1;
  ...
  save(form: NgForm) {
    this.webService
      .saveQuery(this.postData)
      .pipe()
      .subscribe(
        result => {
          this.errorHandlerService.successMessage(
            "Your query successfully submitted. Our executive will be contact within 24 hours."
          );
          form.reset();
        },
        error => {
