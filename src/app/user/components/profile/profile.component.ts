...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.localStorageService.userDetails) {
      ...
      this.loginUser.avatar = this.fileBaseUrl + this.loginUser.id + '/' + this.loginUser.avatar
      this.gerUserBank()
    } else {
      this.logout()
    }
  ...
  savePic(file) {
    ...
    this.userService.uploadProfilePic(formData).pipe().subscribe((result) => {
      ...
      if (result.success == true) {
        ...
        this.errorHandlerService.successMessage('Profile image successfully updated ');
        this.loginUser.avatar = this.fileBaseUrl + this.loginUser.id + '/' + this.loginUser.avatar
      }
      else {
  ...
  submitBank() {
    ...
    if (this.bankDetails.ifsc_code == null || this.bankDetails.ifsc_code.trim() == '') {
      ...
      this.userService.saveBank(this.bankDetails).pipe().subscribe((result) => {
        this.errorHandlerService.successMessage('bank details successfully saved.')
      ...
      this.userService.updateBank(this.bankDetails).pipe().subscribe((result) => {
        this.errorHandlerService.successMessage('bank details successfully updated.')
        this.editBankDetails(false)
        this.gerUserBank()
      }, error => {
        this.errorHandlerService.handleError(error)
