...
import { DateFormatEnum, StatusEnum, SettlementType } from '../../../shared/constants/enum';
...
export class DefaulterComponent implements OnInit {
  ...
  viewDetail(id: string) {
    let sId = btoa(id)
    this.routerService.RedirectDefaulterDetails(sId, this.routerService.userMenu.defaulter)
  }

  editDefaulter(id: string) {
    
    this.defaulterService.checkStatus({defaulter_id:id}).pipe().subscribe((result) => {
      
      if (result.body.status == 'pending' || result.body.status == 'more_details_required') {
        let sId = btoa(id)
  ...
  cancleDefaulter(id: string) {
    this.confirmationDialogService.confirm('Please confirm', 'Are you sure you want to cancel?')
      .then((confirmed) => {
        if (confirmed) {
          this.defaulterService.checkStatus({defaulter_id:id}).pipe().subscribe((result) => {
            ...
            if (result.body.status == 'pending') {
              ...
              //this.defaulterService.removeDefaulterDoc(formData).pipe().subscribe((result_remove_doc) => {
                ...
                this.defaulterService.deleteRequest(id).pipe().subscribe((result) => {
                  this.errorHandlerService.successMessage('Your request has been cancled.');
                  this.getList();
                }, error => {
                  this.errorHandlerService.handleError(error)
                });
